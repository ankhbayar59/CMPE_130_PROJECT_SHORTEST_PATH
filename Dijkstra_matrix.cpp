...
void dijkstra::_EXPLORE_MATRIX(int start_vertex)
{
	...
	for (int i = 2; i < city_number; i++)
	{
		...
		for (int j = 1; j <= city_number; j++)
		{
			if (!isVisited[j] && distance_matrix[temp_next_small_vetrix][j] < INFINITY)
			{
				int NewDistance = track_distance[temp_distance] + distance_matrix[temp_next_small_vetrix][j];
...
void dijkstra::_PRINT_PATH(int start_vertex)
{
	int element_in_path = 1;
	path_list[element_in_path] = city_number;
	element_in_path++;
	for (int i = 1; i <= 5; i++)
	{
		std::cout << prev[i];
	}
	int temp = prev[city_number];
...
void dijkstra::_SHOW_COST_()
{
	...
	double car_cost_money = track_distance[city_number] * 0.2;
	double car_cost_time = track_distance[city_number] / 45;
	double bus_cost_money = track_distance[city_number] * 0.15;
	double bus_cost_time = track_distance[city_number] / 30;
	std::cout << "Car use: /n$" << car_cost_money << " Time: " << car_cost_time << std::endl;
	std::cout << "Bus use: /n$" << bus_cost_money << " Time: " << bus_cost_time << std::endl;
